...
- Four initial banks of exactly twelve addressable pads, at least four velocity layers per pad,
  and 128 deterministic voices. Additional banks may be introduced through a compatible schema
  extension. The twelve-pad presentation follows the supplied original 3x4 brand layout.
...
- Accessible, resizable, original desktop UI for 1280x800 through 4K at 100–200% scaling, using
  the supplied `logo.gif` and twelve-pad geometry without copied third-party product artwork.
- Touch-first iOS and Android layouts, mobile audio-session/lifecycle handling, sandboxed file
  import/share behavior, and mobile permission flows, validated on real devices before release.
