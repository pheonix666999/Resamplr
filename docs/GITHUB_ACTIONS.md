...
`.github/workflows/ci.yml` runs on pull requests, pushes to `main`, and manual dispatch. Default token
permissions are read-only and superseded branch runs are cancelled.

Milestone 0 jobs:

- `validation` on Ubuntu 24.04: formatting/source/schema checks, Debug/Release headless builds, CTest,
  and both smoke paths without hardware or display.
- `windows-x64` on Windows 2025: Release app/tests/smoke and unsigned development archive.
